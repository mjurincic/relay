...
function getParentAndValidateConnection(parentName, connectionName, fatQuery) {
  ...
  !connection.isConnection() ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayMutationQuery: Expected field `%s` on `%s` to be a connection.', connectionName, parentName) : invariant(false) : undefined;
  return parent;
}
